...
- Computer-use / desktop automation tool (revisit for v3).
...
6. **New first-party tools** — WebSearch / WebFetch with allow/deny lists, CodeSandbox, first-class attachments. Safe to add here because the registry is already dynamic and every call is hook/audit covered.
...
8. **Hierarchical compaction + budget-aware attachment injector** — per-turn → session → compact-boundary summaries, selective tool-result trimming, a single token-budget policy informed by the capability layer from #1.
